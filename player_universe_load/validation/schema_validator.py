...
BATTING_STAT_COLUMNS = [
    "player_id", "season_id", "stat_period",
    "G", "AB", "PA", "H", "singles", "doubles", "triples", "HR",
...
PITCHING_STAT_COLUMNS = [
    "player_id", "season_id", "stat_period",
    "GP", "GS", "OUTS", "IP", "TBF", "H", "R", "ER", "HR", "BB",
    "IBB", "K", "HBP", "WP", "BK", "W", "L", "WPCT", "QS",
    "SV", "HLD", "SVHD", "SVO", "BLSV", "SV_pct",
    "ERA", "WHIP", "OBA", "OOBP", "k_bb_ratio", "k_per_9", "bb_per_9",
    "velo", "spin_rate", "eff_min_vel", "percieved_velo", "release_extension",
    "release_pos_x", "release_pos_z", "break_z", "induced_break_z",
    "break_x_arm_side", "break_x_batter_in", "arm_angle",
    "pitcher_run_exp", "pitcher_run_value_per_100",
    "exit_velo", "adj_exit_velo", "launch_angle",
    "swing_miss_pct", "swings", "takes", "whiffs",
    "xAVG", "xOBP", "xSLG", "xwOBA", "xAVGdiff", "xOBPdiff", "xSLGdiff"
]
...
def validate_data_schema(conn, fixtures_dir: Path) -> bool:
    ...
    if hitters_file.exists():
        ...
        if data and len(data) > 0:
            ...
            if "stats" in sample and sample["stats"]:
                if "current_season" in sample["stats"] and sample["stats"]["current_season"]:
                    cs = sample["stats"]["current_season"]
                    if "AB" in cs or "AVG" in cs:
                        is_valid, missing, extra = validate_schema(conn, "player_stats_batting", BATTING_STAT_COLUMNS)
                        if not is_valid:
                            schema_issues.append(("player_stats_batting", missing, extra))
            ...
            if "stats" in sample and sample["stats"]:
                if "current_season" in sample["stats"] and sample["stats"]["current_season"]:
                    ...
                    if "IP" in cs or "ERA" in cs:
                        is_valid, missing, extra = validate_schema(conn, "player_stats_pitching", PITCHING_STAT_COLUMNS)
                        if not is_valid:
                            schema_issues.append(("player_stats_pitching", missing, extra))
    ...
    if schema_issues:
        ...
        for table, missing, extra in schema_issues:
            print(f"📋 Table: {table}")
            if missing:
                print(f"   ⚠️  Missing in DB (need to add these columns):")
                for col in missing:
                    print(f"      - {col}")
            if extra:
                print(f"   ℹ️  In DB but not in data (unused columns):")
                for col in extra:
                    print(f"      - {col}")
            print()
